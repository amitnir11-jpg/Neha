...
function showTabFilter(query = {}) {
  const filter = applyTestScanMode({}, 'real');
  const dealerCode = normalizeDealerCode(query.dealerCode || query.dealer);
  const scanType = upper(query.scanType || query.type);
  const partText = upper(query.partNumber || query.part || query.q);
  const binText = String(query.binLocation || query.bin || '').trim();
  const rawText = upper(query.rawScan || '');

  if (dealerCode) filter.dealerCode = dealerCode;
  if (scanType) filter.$and = (filter.$and || []).concat([{ $or: [{ scanType }, { type: scanType }] }]);
  if (query.dateFrom || query.from || query.dateTo || query.to) {
    const from = new Date(query.dateFrom || query.from || '');
    const to = new Date(query.dateTo || query.to || '');
    filter.timestamp = {};
  ...
  if (partText) {
    const partRegex = new RegExp(escapeRegex(partText), 'i');
    filter.$and = (filter.$and || []).concat([{
      $or: [
        { part: partRegex },
  ...
  if (binText) {
    const binRegex = new RegExp(escapeRegex(binText), 'i');
    filter.$and = (filter.$and || []).concat([{ $or: [{ bin: binRegex }, { binLocation: binRegex }] }]);
  }
  if (rawText) {
    const rawRegex = new RegExp(escapeRegex(rawText), 'i');
    filter.$and = (filter.$and || []).concat([{ $or: [{ rawScan: rawRegex }, { rawScanString: rawRegex }, { rawUpi: rawRegex }] }]);
  }
  return filter;
}
...
async function showTabRows(req, res, format = '') {
  try {
    const page = Math.max(Number(req.query.page || 1), 1);
    const limit = Math.min(Math.max(Number(req.query.limit || 50), 1), 1000);
    const filter = showTabFilter(req.query);
    ...
    const summaryRows = allForSummary.map(publicScan);
    const summary = summaryRows.reduce((acc, row) => {
      const rowQty = Number(row.qty || 0);
      acc.totalQuantity += rowQty;
      const type = upper(row.scanType || row.type);
      if (type === 'INWARD') acc.totalInward += rowQty;
      if (type === 'OUTWARD') acc.totalOutward += rowQty;
      if (type === 'FITTED') acc.totalFitted += rowQty;
      if (type === 'DAMAGE') acc.totalDamage += rowQty;
      return acc;
    }, { totalRecords, totalQuantity: 0, totalInward: 0, totalOutward: 0, totalFitted: 0, totalDamage: 0 });

    const exportRows = format ? summaryRows : rows;
    if (format === 'csv') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="Daksh_SHOW_TAB.csv"');
      return res.send(csv);
    }

    if (format === 'excel') {
      const workbook = new ExcelJS.Workbook();
      const sheet = workbook.addWorksheet('SHOW TAB');
      sheet.columns = [
        { header: 'Scan Time', key: 'timestamp', width: 22 },
        { header: 'Dealer Code', key: 'dealerCode', width: 16 },
        { header: 'Dealer Name', key: 'dealerName', width: 24 },
        { header: 'Part Number', key: 'partNumber', width: 20 },
        { header: 'Raw Scan', key: 'rawScan', width: 54 },
      ...
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      res.setHeader('Content-Disposition', 'attachment; filename="Daksh_SHOW_TAB.xlsx"');
      await workbook.xlsx.write(res);
      return res.end();
    }

    return res.json({ success: true, records: rows, summary, page, limit, totalPages: Math.max(Math.ceil(totalRecords / limit), 1) });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
}

router.get('/show-tab', auth.requireAuth, (req, res) => showTabRows(req, res));
router.get('/show-tab/export.csv', auth.requireAuth, (req, res) => showTabRows(req, res, 'csv'));
router.get('/show-tab/export.xlsx', auth.requireAuth, (req, res) => showTabRows(req, res, 'excel'));
