...
const REMOVED_REPORT_COLUMN_KEYS = new Set([
  'lastMovementDate',
  'movementQtyLast30Days',
  'movementQtyLast90Days',
  'movementQtyLast180Days',
  'movementQtyLast365Days',
  'saleQtyLast12Months',
  'salesQtyLast12Months',
  'movementCode',
  'movementCodeA',
  'movementCategory',
  'partMovement',
...
const REMOVED_REPORT_HEADER_PATTERN = /last\s+movement|movement\s+qty|sale\s+qty|sales\s+qty|movement\s+code|part\s+movement|price\s+ageing|inventory\s+risk\s+value/i;

function isRemovedReportColumn(column = {}) {
  return REMOVED_REPORT_COLUMN_KEYS.has(String(column.key || '').trim()) || REMOVED_REPORT_HEADER_PATTERN.test(String(column.header || ''));
}

function stripRemovedReportFields(row = {}) {
  const cleanRow = { ...row };
  REMOVED_REPORT_COLUMN_KEYS.forEach((key) => {
    delete cleanRow[key];
  });
  return cleanRow;
}
...
function rowValueSummary(scans = [], catalogue = {}, priceHistories = []) {
  ...
  const totalValue = money(summary.totalQty * masterMrp);
  return {
    currentCatalogueMRP: masterMrp,
    totalQty: summary.totalQty,
    scanUPIMRP: masterMrp > 0 ? masterMrp : '',
    averageScannedMRP: masterMrp,
    minScannedMRP: masterMrp,
    maxScannedMRP: masterMrp,
    totalScanValue: 0,
    totalManualValue: 0,
    finalInventoryValue: totalValue,
    scannedQty: 0,
    manualQty: 0,
    priceChangeCount: masterMrp > 0 ? 1 : 0,
...
function latestPriceForReport(priceHistories = [], asOf = new Date()) {
  return latestCurrentPriceFromRows(priceHistories, asOf) || null;
}
...
function latestMrpEffectiveDate(priceHistories = []) {
  const latestPrice = latestPriceForReport(priceHistories);
  return latestPrice ? formatDate(latestPrice.effectiveFrom) : '';
}

function inventoryRiskAction(status = '') {
  if (status === 'MRP Pending') return 'Update MRP';
  if (status === 'Excess') return 'Verify excess physical/fitted quantity';
  if (status === 'Short') return 'Verify shortage against system quantity';
...
function normalizeCategory(value) {
  return canonicalizePartCategory(value);
}
...
function reportCategory(row = {}) {
  return displayCategory(row.productCategory || '');
}
...
function partwiseStatus({ mrp = 0, physicalQty = 0, fittedQty = 0, systemQty = 0 }) {
  return auditStockStatus({ mrp, physicalQty, fittedQty, systemQty });
}

function partwiseAction(varianceQty, hasSystemStock, physicalQty, status = '') {
  if (status === 'MRP Pending') return 'Update MRP';
...
function applyVarianceFilter(rows, varianceType) {
  switch (cleanText(varianceType).toLowerCase()) {
    case 'matched':
      return rows.filter((row) => ['Matched', 'Inventory Matched'].includes(row.status));
    case 'short':
...
function addSheet(workbook, name, columns, rows) {
  ...
  sheet.columns = activeColumns.map((column) => ({
    header: column.header,
    key: column.key,
    width: column.width || 18
  }));
  rows.forEach((row) => {
    const formatted = stripRemovedReportFields(row);
...
function selectedReportColumns(columns = [], query = {}) {
  ...
  const activeColumns = (columns || []).filter((column) => !isRemovedReportColumn(column));
  if (!selected.length) return activeColumns;
  const byKey = new Map(activeColumns.map((column) => [column.key, column]));
  const ordered = selected.map((key) => byKey.get(key)).filter(Boolean);
  return ordered.length ? ordered : activeColumns;
}
...
async function buildLegacyReportData(query = {}) {
  ...
  const rows = masterParts.map((part) => {
    ...
    return {
      ...
      model: part.model,
      year: part.year,
      category: displayCategory(part.productCategory || part.category || ''),
      productCategory: displayCategory(part.productCategory || part.category || ''),
      bin: binLocation,
      binLocation,
      mrp: displayMrp,
  ...
  scanTotals.forEach((physicalQty, partNo) => {
    ...
    const category = displayCategory(master ? (master.productCategory || master.category || '') : '');
  ...
  finalRows.forEach((row) => {
    ...
    if (['Matched', 'Inventory Matched'].includes(row.status)) item.matched += 1;
  ...
  return {
    ...
    binRows: finalRows.map((row) => ({
      ...
      productCategory: displayCategory(row.productCategory || ''),
...
function mainAuditColumns() {
  return [
    ...
    { header: 'DAMAGE QTY', key: 'damageQty', width: 14, numFmt: '#,##0.00' },
    { header: 'INWARD QTY', key: 'inwardQty', width: 14, numFmt: '#,##0.00' },
...
function mainAuditRows(data) {
  return data.finalRows.map((row) => ({
    ...
    mrp: row.mrp,
    dlc: row.dlc,
    latestMrpEffectiveDate: row.latestMrpEffectiveDate || '',
    physicalQty: row.physicalQty,
...
function countingRows(data) {
  return data.scans.map((scan) => ({
    ...
    landedCost: Number(scan.currentCatalogueMRP || 0),
...
function masterCategory(master = {}) {
  return resolveCategoryFromMaster(master);
}
...
function rowCategory(scan = {}, master = {}) {
  return displayCategory(masterCategory(master));
}
...
function enrichScan(scan = {}, master = {}, system = {}) {
  ...
  const binLocation = scan.binLocation || scan.bin || system.binLocation || system.bin || master.binLocation || master.bin || '';
  const year = hasMaster ? master.manufacturingYear || master.year || '' : '';
  const qty = Number(scan.qty || scan.quantity || 0);
  const partDescription = hasMaster ? unknownIfBlank(rowDescription(scan, master)) : MASTER_NOT_FOUND_LABEL;
  const category = hasMaster ? rowCategory(scan, master) : 'Uncategorized';
  ...
  const enriched = {
    ...
    bin: binLocation,
    binLocation,
    mrp: Number(scan.currentCatalogueMRP || scan.valuationMRP || 0),
    ...
    dmsQty: hasSystem ? masterQty(system) : 0,
    systemQty: hasSystem ? masterQty(system) : 0
  };
  return enriched;
}
...
function finalReportCsv(rows = []) {
  const columns = [
    ...
    ['Product Category', (row) => displayCategory(row.productCategory || '')],
    ['MRP', (row) => row.mrp || ''],
    ['DLC', (row) => row.dlc || ''],
...
function buildAuditRow(group, master = {}, system = {}, priceHistories = []) {
  ...
  const hasCatalogue = Boolean(master && (master.partNo || master.partNumber || master.normalizedPartNumber));
  const hasSystemStock = Boolean(system && (system.partNo || system.partNumber || system.normalizedPartNumber || system.dmsStock !== undefined || system.systemQty !== undefined || system.openingStockQty !== undefined || system.quantity !== undefined));
  const dmsQty = hasSystemStock ? systemQtyValue(system) : 0;
  ...
  const systemBins = splitBins((system && (system.binLocation || system.bin)) || master.binLocation || master.bin || '');
  const partDescription = hasCatalogue ? unknownIfBlank(pricing.partDescription || master.partDescription || master.partName || '') : MASTER_NOT_FOUND_LABEL;
  const category = displayCategory(pricing.productCategory || master.productCategory || master.category || '');
  ...
  const row = {
    ...
    partName: partDescription,
    partDescription,
    model: pricing.model || (hasCatalogue ? master.model || '' : ''),
    year: pricing.year || (hasCatalogue ? master.manufacturingYear || master.year || '' : ''),
    manufacturingYear: pricing.manufacturingYear || (hasCatalogue ? master.manufacturingYear || master.year || '' : ''),
    category,
    productCategory: category,
    ...
    dlc,
    productGroup: pricing.productGroup || (hasCatalogue ? master.productGroup || '' : 'UNKNOWN'),
    partSubGroup: pricing.partSubGroup || (hasCatalogue ? master.partSubGroup || master.productSubGroup || '' : 'UNKNOWN'),
    dmsQty,
    systemQty: dmsQty,
    physicalQty,
    binPhysicalQty,
    ...
    totalAuditQty: auditedQty,
    reservedQty: Number((system && system.reservedQty) || master.reservedQty || 0),
    physicalBin1: physicalBins.bin1,
    physicalBin2: physicalBins.bin2,
    physicalBin3: physicalBins.bin3,
    otherBinLocations: physicalBins.otherBins,
...
async function buildReportData(query = {}) {
  ...
  let scans = rawScans.map((scan) => {
    ...
    return enrichScan(
      scan,
      masterByDealer.get(masterKey(partNo, dealerCode)) || masterByPart.get(partNo) || null,
      systemByPart.get(partNo) || null
    );
  });
...
function partwiseRowFrom(partNo, group = {}, catalogue = {}, system = {}, priceHistories = []) {
  ...
  const valueSummary = rowValueSummary(scans, hasCatalogue ? catalogue : {}, priceHistories);
  ...
  const action = partwiseAction(varianceQty, hasSystemStock, physicalQty, status);
  ...
  const binLocations = Array.from(new Set([
    ...
    ...splitBins(firstPresent(system.binLocation, system.bin) || ''),
  ...
  return {
    partNum: partNo,
    partNumber: partNo,
    partDescription: hasCatalogue ? unknownIfBlank(pricing.partDescription || catalogue.partDescription || catalogue.partName || '') : MASTER_NOT_FOUND_LABEL,
    productCategory: displayCategory(pricing.productCategory || catalogue.productCategory || catalogue.category || ''),
    productGroup: hasCatalogue ? unknownIfBlank(pricing.productGroup || catalogue.productGroup || '') : 'UNKNOWN',
    partSubGroup: hasCatalogue ? unknownIfBlank(pricing.partSubGroup || catalogue.partSubGroup || catalogue.productSubGroup || '') : 'UNKNOWN',
    model: hasCatalogue ? (pricing.model || catalogue.model || '') : '',
    year: hasCatalogue ? (pricing.year || catalogue.manufacturingYear || catalogue.year || '') : '',
    mrp,
    scanUPIMRP: valueSummary.scanUPIMRP,
    currentCatalogueMRP: pricing.mrp,
    averageScannedMRP: valueSummary.averageScannedMRP,
    ...
    pricePeriod: valueSummary.pricePeriod,
    latestMrpEffectiveDate: masterPriceDateText(catalogue),
    dlc,
    ...
    varianceOnMrp: varianceValueOnMrp,
    varianceOnDlc: pricing.varianceStockValue,
    reservedQty: reservedQtyValue(system) || reservedQtyValue(catalogue),
    action,
    inventoryRiskStatus: status,
    actionRemarks: hasCatalogue ? inventoryRiskAction(status) : MASTER_NOT_FOUND_LABEL,
    ...
    lastScanTime: group.lastScanTime || '',
    status,
    masterStatus: hasCatalogue ? 'MASTER FOUND' : 'MASTER NOT FOUND'
  };
}
...
async function masterMapsForScans(scans = []) {
  ...
  const masterByDealer = new Map();
  const masterByPart = new Map();
  const masters = partNumbers.length ? await getPricesFromPartMaster(partNumbers) : new Map();
  masters.forEach((master) => {
    const partNo = masterPartNumber(master);
    const dealerCode = cleanText(master.dealerCode).toUpperCase();
    if (partNo && dealerCode) masterByDealer.set(masterKey(partNo, dealerCode), master);
...
async function buildCategoryWiseVarianceSummary(query = {}) {
  ...
  rawScans.forEach((scan) => {
    ...
    const category = master ? displayCategory(resolveCategoryFromMaster(master)) : 'Uncategorized';
    ...
    if (action !== 'Inventory Matched' && (!actionFilter || actionFilter === 'Inventory Matched')) {
      addCategoryVarianceGroup(groupMap, category, 'Inventory Matched', 0, mrp, dlc);
    }
  });
...
function stockSummaryRowMatchesFilters(row = {}, query = {}) {
  const category = cleanText(query.productCategory || query.category);
  if (category && !new RegExp(escapeRegExp(category), 'i').test(row.productCategory || '')) return false;
...
function stockSummaryDlcRate(row = {}) {
  return firstPositiveNumber(row.currentCatalogueDLC, row.dlc);
}
...
function stockSummaryMatrixRows(rows = []) {
  const knownCategories = STOCK_SUMMARY_CATEGORIES.slice();
  const extraCategories = Array.from(new Set(rows.map((row) => stockSummaryCategory(row)).filter((category) => category && !knownCategories.includes(category))));
  const categoryOrder = knownCategories.concat(extraCategories);
  const buckets = new Map(categoryOrder.map((category) => [category, emptyStockSummaryCategory(category)]));
  rows.forEach((row) => {
  ...
  return categoryOrder.map((category) => roundStockSummaryMatrixRow(buckets.get(category) || emptyStockSummaryCategory(category)));
}
